...
{    
    ...
    public ILogger<GetVideosQueryHandler> Logger { get; }
    public IKernel Kernel { get; }
    public IReadRepository<Video> VideoRepository { get; }

    public async Task<Page<VideoDTO>> Handle(GetVideosQuery request, CancellationToken cancellationToken)
    {
        // TODO: hack and I am ignoring a ton of request parameters
        var list = new HashSet<string>();

        var srcText = request.SearchText;
        ...
        {            
            // YOUTUBE:tMwLRbX66Jo:65
            var idParts = x.Metadata.Id.Split(':');
            list.Add(idParts[1]);

            Logger.LogDebug("Found [{Relevance}] match on [{Description}]", x.Relevance, x.Metadata.Description);
        }

        var res = await VideoRepository.ListAsync(new QueryVideos.ByProviderItemIds("YOUTUBE", list), cancellationToken);

        var final = res.Select(v => new VideoDTO(
            v.Id,
            ...
            -1,//dbContext.Artifacts.Count(a => a.VideoId == v.Id),
            -1,//dbContext.Transcripts.Count(a => a.VideoId == v.Id),
            v.Tags.Count(),
            v.Origin.ProviderId,
        ...
        return new Page<VideoDTO>(final, 0, request.Take ?? 10, list.Count);
    }
